...
```acceptance-report
{
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Read all 10 source/test/doc files. Identified exact root causes, file paths, line ranges, and existing test patterns for both bugs."
    }
  ],
  "changedFiles": [],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "grep, find, read across src/extensions/workflow/, src/__tests__/, docs/",
      "result": "passed",
      "summary": "Mapped full workflow codebase: 10 files, ~2000 lines"
    }
  ],
  "validationOutput": [
    "Bug 1: marker validators exist and work, but AI may not follow prompt instructions; subagent text fallback can write invalid artifacts",
    "Bug 2: tool_result handler has no dedup; every bash/subagent result triggers onArtifactMaybe; loop orchestration only guards on agent name"
  ],
  "residualRisks": [
    "Bug 1: shouldReplaceInvalidArtifact only replaces when fallback is valid — an invalid subagent output leaves a bad artifact in place, stalling the workflow until the model manually fixes it",
    "Bug 2: parallel subagent calls returning around the same time can each trigger a separate followUp, flooding the model with duplicate instructions",
    "Loop orchestration does not track which tool_call_ids it has processed — a retried subagent call could re-trigger loop state transitions"
  ],
  "noStagedFiles": true,
  "diffSummary": "No code changes — pure exploration and analysis",
  "reviewFindings": [
    "blocker: adapter.ts:300-330 — subagent text fallback can write artifact without required marker, stalling workflow",
    "blocker: adapter.ts:280-370 — tool_result handler fires on every bash/subagent result with no dedup, causing duplicate followUp queuing",
    "minor: adapter.ts:340-370 — loop orchestration only guards on agent name, non-builder/commentator subagent calls in loop phase fall through to onArtifactMaybe (noOp, but wasteful)",
    "info: state-machine.ts:230-250 — reentrancy guard is correct but only prevents concurrent calls, not sequential duplicate checks",
    "info: validators.ts:15-30 — marker regex is correct and case-insensitive"
  ],
  "manualNotes": "Both bugs are in adapter.ts. The state machine (state-machine.ts) and validators (validators.ts) are correct. Fixes should: (1) make subagent text fallback more aggressive about replacing invalid artifacts, (2) add toolCallId dedup to tool_result handler, (3) skip non-phase-relevant tool results. Existing tests in adapter.test.ts and state-machine.test.ts provide good coverage to validate fixes."
}
```
