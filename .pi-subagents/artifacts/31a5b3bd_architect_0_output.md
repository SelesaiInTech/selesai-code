...
```acceptance-report
{
  "criteriaSatisfied": [
    {
      "id": "criterion-1",
      "status": "satisfied",
      "evidence": "Plan confines implementation to existing interactive update path and one focused regression test; no lifecycle, RTK, type, packaging, or dependency changes."
    }
  ],
  "changedFiles": [],
  "testsAddedOrUpdated": [],
  "commandsRun": [
    {
      "command": "rg/read inspection of RTK extension, AgentSession event forwarding, interactive handler, bash tool, and existing tests",
      "result": "passed",
      "summary": "Identified existing updateArgs API and exact omitted call."
    },
    {
      "command": "rtk rewrite and shell-output comparison",
      "result": "passed",
      "summary": "Confirmed RTK executes rewrites; defect limited to stale displayed command header."
    }
  ],
  "validationOutput": [
    "No implementation performed; plan includes runnable targeted, full-suite, build, whitespace, and status checks."
  ],
  "residualRisks": [
    "Header refresh depends on tool emitting tool_execution_update; built-in bash does emit an initial update before output."
  ],
  "noStagedFiles": true,
  "diffSummary": "No diff created; planned one-line interactive event-consumption fix plus one regression test.",
  "reviewFindings": [
    "no blockers"
  ],
  "manualNotes": "RTK extension and installed bundled manifest were verified present; runtime rewriting works."
}
```

WORKFLOW_PLAN_STATUS: ready

⧉ copy assistant: /cp b93238