...
- [x] 8g. Document the cost RPC data contract and its lower-bound semantics.
...
**Current checkpoint at `d0e4a0889`:** extension typecheck passes; serialized full unit suite passes (2,937 passed, 12 skipped); serialized full integration suite passes (990 passed); root `npm run build` and `npm test` pass (59 files, 1,083 tests). Earlier parallel suite timeouts were reproduced as isolated passes; this checkpoint is green.
