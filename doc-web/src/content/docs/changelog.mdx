...
## 0.13.33 — Subagent cost RPC, terminal proof, and safer children

- **Subagent cost is available as data.** The versioned subagent RPC surface gains a `cost` method that returns the same parent-plus-child accounting `/subagent-cost` prints, and `ping` advertises the report version. It reads the current session branch and existing run artifacts, so callers should ask on turn boundaries rather than on a timer.
- **Async workflow status carries terminal proof.** `subagent({ action: "status" })` details include `workflowTerminalProof`, the shared child-exit evidence for workflow runs, so a caller can assert how a workflow actually ended.
- **Native children can use the scoped capability gateway.** When extension policy permits, a child launch exposes `capability_catalog`, `capability_discover`, and `capability_skill_show` — limited to tools visible in that child's effective registry. The gateway never installs or provides a missing tool provider, and `SELESAI_CAPABILITY_GATEWAY=0` still disables it everywhere.
- **Packaged `worker` starts fresh.** The builtin `worker` agent now defaults to fresh context (`acceptanceRole: writer`) instead of a forked session; an explicit `context: "fork"` still wins.
- **Gateway controls do not disarm the implementation guard.** A read-only child that also receives `capability_catalog`, `capability_discover`, and `capability_skill_show` is still refused an implementation task with no mutation-capable tools; the gateway only searches, activates, or loads capabilities inside the child's existing tool policy.
- **Failed structured output keeps its evidence.** Foreground and background runs report the bounded rejection diagnostic — schema and validator failures summarized, submitted values redacted — instead of a generic missing-call error, and background results carry `structuredOutputFailed`.
- **`subagent_supervisor` requires fanout authorization.** A child that declares the supervisor reply tool without `subagent` in its effective tools (or `allowNestedSubagents`) now fails preflight instead of silently getting the tool.
- **Child processes no longer inherit Git routing variables.** Background runners and external CLI default launches drop `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, and Git's other local environment, so a child's Git commands stay in its own working directory.

## 0.13.32 — Cached provider model catalogs stay available

- **Cached model catalogs survive incomplete refresh metadata.** When a provider catalog response has no usable `Last-Modified` value, Selesai now uses the completed cache-check time instead of discarding newer cached models. OpenAI Codex models such as `gpt-6-sol` and `gpt-6-luna` therefore remain available in `/model` and `--list-models` after refresh.

## 0.13.31 — Default-on Jev capability tie-breaking

- **Jev routing is on by default.** The capability gateway asks Jev only to break ambiguous hints between two or three optional tools; toggle it in `/settings`. Token-In credentials are required. Without them, no Jev request is sent and Selesai prompts you to run `/tokenin add`; deterministic routing remains the fallback.

## 0.13.30 — Jev advisory routing and bundled engineering skills

- **Opt-in advisory routes.** The bundled `jev-advisory-routing` extension can make one bounded, read-only local memory lookup after an explicit durable-memory cue, and suggest one discovered skill/workflow or a proportionate verification level. Both routes are off by default; they never write memory, load a skill, start a workflow, run a command, or weaken required checks.
- **Opt-in capability tie-breaking.** For an ambiguous lexical hint between two or three optional tools, the capability gateway can ask Jev which one to expose. Jev sees only the bounded current prompt and compact candidate lines — not the full catalog, schemas, or history — and every unavailable, invalid, low-confidence, or `none` answer leaves deterministic routing unchanged.
- **Reliable Jev decisions.** Advisory routing and capability tie-breaking share the Token-In decisions transport. `jev-*` calls use its non-streaming endpoint and rotateable-account failover.
- **Four engineering skills bundled by default.** `planning-and-task-breakdown`, `incremental-implementation`, `code-review-and-quality`, and `code-simplification` now ship and seed into the agent directory. Their shared reference material and upstream MIT license ship with them.

## 0.13.29 — MiMo V2.6 in the Token-In catalog

- **Two MiMo V2.6 models join the bundled catalog.** `mimo-v2.6-pro` and `mimo-v2.6-flash` are available out of the box through the Token-In gateway: 1M context, 128K max output, text and image input, and reasoning. Your own `models.json` entries are unaffected.

## 0.13.28 — Automatic model routing

Every idle prompt can now pick its own model. The new bundled `auto-model` extension asks the Jev decisions model (`jev-1.13`, through the Token-In gateway) how hard the request is, then switches to the model you configured for that tier.

- **Four tiers, your models.** `settings.json` gains an `autoModel` block with a classifier and one model each for `simple`, `medium`, `complex`, and `reasoning`. Disabled by default.
- **Tier values are `provider/model`.** Add an optional `:thinkingLevel` suffix (`tokenin/celestial-max:max`) and the extension also sets the thinking level.
- **Only idle top-level prompts change model.** Queued steering and follow-up messages, extension-injected messages, and `/` commands are left alone, because the model is session-global. Choosing a model with `/model` suspends auto routing for the rest of the session.
- **Failures keep you running.** A classifier timeout, a malformed or unsure answer, a missing or out-of-scope model, or unavailable credentials keeps the current model or falls back to `autoModel.fallbackTier` (default `medium`).

**Graft**

- **Deep-build model default for existing installs.** Startup now seeds `graft.deepModel` (`deepseek-v4.1-flash`) into an existing settings file when it is absent, without overwriting a model you chose.

## 0.13.27 — Pi v0.86.1 sync

Ported the coding-agent source from Pi v0.85.1 to v0.86.1, and bumped `pi-ai`, `pi-agent-core` and `pi-tui` to 0.86.1. The package now requires Node 22.19.0 or newer.

**Adopted from upstream**

- **Custom-provider request contract.** Stream handlers now receive a normalized transcript, and must call the outgoing-payload hook before sending and the response hook before consuming the body. Selesai's providers never read the request context, so this landed at the forwarding boundary — your `models.json` configuration and extension-registered providers are unaffected.
- **Sectioned system prompt.** A session records the prompt and tool changes it ran under, so resuming a session or visiting an earlier branch restores the instructions that were actually in effect, and your prompt cache stays warm.
- **Whole-prompt replacement.** An extension can replace the instruction for a turn instead of only appending to it, so presets and personas work.
- **Per-model compaction budgets.** Reserved and kept-recent budgets can be set per model.
- **Session runtime rebase.** Cancelling a compaction while credentials are still resolving is no longer ignored; a prompt arriving mid-stream is queued instead of raced; provider request settings apply uniformly to main turns, compaction and extension model calls; and the retry-backoff cap is configurable.
- **Strict JSON-schema sampling by default**, with an opt-out for tools whose schema cannot be expressed strictly.
- **Wider default tool set** including the search and listing tools, each still individually disableable.
- **Extension authoring.** Event subscriptions return an unsubscribe handle; tool-call arguments and results are restricted to JSON-compatible values; shell hooks fail loudly instead of silently; and the embedded-module map now branches on runtime kind.
- **Interactive workspace.** Deferred extension-catalogue refresh, new settings surfaces, and the crash notice and bug report.

**Fork fixes**

- **Clipboard failures are surfaced** instead of swallowed.
- **Overflow recovery keeps its guard**, so a response stopped at the output limit does not re-arm recovery into a loop.
- **A doc comment naming `~/.pi/agent/sessions`** now names `.selesai/agent/sessions`.

**Dependency removed**

- **`@mariozechner/clipboard` retired.** Clipboard now uses the helpers bundled with `pi-tui`, so installs are smaller with one fewer native dependency. Paste and copy-on-select are unchanged.

**Not adopted**

- **Mermaid rendering** is not adopted; no rendering dependency or setting was added.
- **Fork behaviour is unchanged:** automatic handoff, bounded length continuation, the vision caption relay, the compaction-failure event and the capability gateway are all re-planted on the new upstream shapes.

## 0.13.26 — Goal-preserving auto handoff and refreshed model catalog

- **Automatic handoff keeps the big picture.** Auto-handoff now runs with the same goal text as `/handoff-new`, so the continuation prompt carries the overall objective, success criteria, key decisions, reference files, completed and remaining work, and the next concrete action.
- **Failed handoffs report the real error.** A failed or empty handoff generation now shows the underlying error instead of "Cancelled" and rejects the command; the automatic handoff retries on the next settled turn while the session stays above the threshold.
- **RPC handoff keeps the session name.** `handoff_new` now carries the current session name into the replacement session, matching `/handoff-new`.
- **Default model catalog refreshed.** Added `glm-5.3` (1M context), `kimi-k3` (512K), and `gemini-3.7-flash`; `celestial-pro` accepts images; `deepseek-v4.1-flash` uses 64K max output tokens. `deepseek-v4-flash`, `deepseek-v4-flash-vision-exp`, `kimi-k2.7-code`, and `glm-5.2` are retired from the bundled catalog (`glm-5.3` supersedes `glm-5.2`).

## 0.13.25 — Graft deep-build fallback

- **Graft retries usable scoped models.** When the active model cannot produce Graft-parsable summaries, the deep build tries up to two other models scoped to that provider before reporting the failure.
- **Pin the deep-build model when needed.** Set `graft.deepModel` to use one active-provider model and skip fallback candidates.

## 0.13.24 — Graft builds itself and deep builds use the active model

- **Graft builds its own graph.** A fresh repository now gets its initial graph automatically—at session start, on the first read-only tool call, and after a CLI install—instead of waiting for a manual build. A compatible active Selesai model (TokenIn, LiteLLM, OpenAI, Anthropic, or OrcaRouter) adds the deep tier; otherwise the build stays local and structural.
- **Deep builds use the active Selesai model.** Deep enrichment no longer needs Graft's own provider configuration: the active model's credential is passed only to the Graft child process for that build, never through the host environment.
- **Default retrieval mode is now `hybrid`.** Eligible coding turns start from a smaller orientation pack while precise Graft tools stay available; `graft.mode: "push"` restores the previous full-pack behavior.
- **Deep is the default build.** `/graft build` and `/graft setup` run the provider-backed deep build, and `/graft build --structural` stays local and offline.

## 0.13.23 — Bundled Graft code context

- **Native Graft integration.** Selesai now bundles `@nanonets/graft` code context: it installs a compatible CLI when missing, builds a consent-gated local graph, injects bounded source-backed context for eligible coding turns, and exposes six read-only semantic retrieval tools plus `/graft` lifecycle commands. Structural builds stay local and offline; deep provider-backed builds require separate confirmation.
- **Graft-aware builtin subagents.** `scout`, `reviewer`, `worker`, `delegate`, and `oracle` automatically receive Graft's read-only code-context tools and child-only extension when Graft is loaded. Explicit user tool or extension overrides retain precedence.
- **Gateway keeps Graft active.** The progressive capability gateway leaves Graft's code-context tools active so pre-turn retrieval and precise follow-ups continue to work.

## 0.13.22 — Live TPS gate and quieter capability routing

- **Live TPS readings are gated.** The live tokens-per-second status now waits for a usable sample window and drops implausible rates, instead of showing a burst of early tokens divided by almost no elapsed time.
- **Capability gateway hint noise removed.** The gateway no longer injects fuzzy skill recommendations or ambiguous-match catalog hints into the model context. A uniquely matched tool is still activated automatically, and optional capabilities remain discoverable through `capability_catalog`.
- **Capability catalog tool rows.** `capability_catalog` calls now show the search query and optional kind in the TUI.
...
- **Typo-tolerant capability routing.** A tool or skill name one character off from the catalog (e.g. `inercom` → `intercom`) now scores as an exact match, so a uniquely identifiable typo auto-activates the tool instead of returning a hint.
