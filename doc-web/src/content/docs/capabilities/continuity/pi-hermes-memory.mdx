...
- The optional Jev advisory memory route can request one bounded, read-only local `memory_search` after an explicit durable-memory cue; it never receives memory contents or writes memory.
