...
Optional settings in `~/.selesai/agent/settings.json` under the `rewind` key:

| Setting | Type | Description |
| --- | --- | --- |
| `rewind.silentCheckpoints` | boolean | Hide footer/status checkpoint notifications. |
| `rewind.retention.maxSnapshots` | number | Cap on unpinned unique snapshot commits kept reachable. |
| `rewind.retention.maxAgeDays` | number | Age limit for unpinned snapshot commits. |
| `rewind.retention.pinLabeledEntries` | boolean | Keep snapshots bound to labeled session-tree nodes exempt from pruning. |
| `rewind.retention.scanMode` | `"ancestor-only"` or `"repo-sessions"` | Discovery mode for retention sweeps; default `ancestor-only`. |
| `rewind.retention.startupBudgetMs` | number | Startup time budget for retention sweeps; if exceeded, the sweep is skipped. |

If `rewind.retention` is omitted, exact history is kept indefinitely. This can grow git object storage without bound.
