...
The `auto-session-name` extension names the session on every user message using the currently selected model. It sends only user-message history (no tool results, assistant replies, or thinking), caps the name at five words, and runs in the background so it never delays your message.
...
- Keep session names current automatically as the conversation topic changes.
- See the name update without running `/name` manually after each message.
