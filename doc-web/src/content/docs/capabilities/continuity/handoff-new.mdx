...
- Runs automatically when configured: with `autoHandoff.enabled` and the context above `autoHandoff.thresholdTokens`, the TUI triggers the same command at the end of a turn (once per threshold crossing, retrying on the next settled turn if generation fails).
...
- Run `/handoff-new` to continue from a clean session after the model summarizes the current thread. The new session keeps the current session's name (you can rename it with `/name`).
...
- Handoff generation requires a working model and API key. If generation fails or returns no text, the error is reported, the command rejects, and no new session is created.
