...
- Starts a local broker on demand and publishes presence (name, cwd, model, status) for connected sessions.

## What you can configure

Configuration lives in `~/.selesai/agent/intercom/config.json`. Defaults:

| Setting | Default | Description |
| --- | --- | --- |
| `brokerCommand` | `npx` | Command used to start the broker process. |
| `brokerArgs` | `["--no-install", "tsx"]` | Arguments passed before the broker script. |
| `enabled` | `true` | Enable or disable intercom for this session. |
| `replyHint` | `true` | Include the exact `intercom({ action: "reply", ... })` call in incoming messages. |
| `confirmSend` | `false` | Show a confirmation dialog before non-reply sends from TUI sessions. |
| `status` | — | Optional custom status suffix appended to the automatic lifecycle status. |
...
- Reply to the most recent inbound ask with `intercom({ action: "reply", message: "..." })`.
...
- Same-machine only. The broker uses Unix domain sockets (macOS/Linux) or a named pipe (Windows); there is no network transport.
