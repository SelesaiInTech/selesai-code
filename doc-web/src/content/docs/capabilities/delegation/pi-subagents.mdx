...
- The bundled workflow extension can drive a `workflowScript` as a phase step; `workflowScript` stays child-level/ephemeral and is not resumable as a process — pair it with the workflow extension when durable, resumable, human-gated execution is needed.
