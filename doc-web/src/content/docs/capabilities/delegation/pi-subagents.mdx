...
- Registers inline agent invocation: `#agent-name` at the start of a message runs that agent directly, and typing `#` in the editor autocompletes installed agents.
...
- Invoke a single agent inline: `#architect Turn this plan into a step-by-step implementation plan.` — `#agent-name` at the start of a message runs that agent directly (like `/run`). Unknown or ambiguous agent names produce a notification and consume the input without sending anything to the main agent.
...
- In Fleet inspector, steer the selected live child with `s` or stop its top-level async run with `D` and confirmation.
...
| `#<agent> [task]` | Inline form of a single-agent run: `#agent-name` at the start of a message runs that agent directly. |
