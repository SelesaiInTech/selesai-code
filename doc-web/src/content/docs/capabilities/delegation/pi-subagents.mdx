...
- When bundled Graft is loaded, gives the code-facing builtin agents (`scout`, `reviewer`, `worker`, `delegate`, `oracle`) its read-only context tools and child-only provider automatically; explicit user overrides still win.
- Sets `SELESAI_SUBAGENT_PARENT_SESSION` in child processes for permission-system forwarding.
- Gives native children the scoped capability gateway when extension policy permits it, so a child can search, activate, and load optional capabilities from its own effective tool registry.
...
| `worker` | Implement approved work and validate results; defaults to fresh context (`defaultContext: fresh`). |

## Limits and safety

- Child agents run in the same host environment as the parent; trust boundaries are the same.
- Spawned children do not receive the bundled `pi-subagents` skill by default, and their tool visibility is controlled by the agent's `tools` frontmatter. A child only gets a child-safe `subagent` tool if its resolved `tools` includes `subagent`; the supervisor reply tool `subagent_supervisor` additionally requires effective fanout authorization (`subagent` in the tools allowlist or `allowNestedSubagents`).
- A child's capability gateway is scoped to that child's effective registry: it catalogs and activates only tools the child can already see, never installs or provides a missing tool provider, and respects `SELESAI_CAPABILITY_GATEWAY=0`.
