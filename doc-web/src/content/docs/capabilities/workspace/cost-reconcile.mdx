...
- Providers whose payloads carry no cost (first-party OpenAI/Anthropic) record nothing; their rate-card estimate matches the bill when catalog prices are configured.

## What it sets up

- Installs a tee-wrapping `fetch` once per process that scans LLM API responses for provider-reported cost and response ids.
- On `message_end`, writes a `cost-reconcile` custom session entry per response id (bounded per session).
...
- Capture is best-effort: the response body is tee'd and scanned in the background; the main response stream is never altered.
