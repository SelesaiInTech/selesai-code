...
- Graft code-context tools also remain active so their pre-turn context and precise follow-up tools keep working.
...
- Exposes the same three tools to native subagent children when extension policy permits it, scoped to that child's own effective tool registry.
...
- In a native subagent child, the gateway catalogs and activates only tools visible in that child's effective registry. It never installs or provides a missing tool provider, and `SELESAI_CAPABILITY_GATEWAY=0` disables it in children too.
...
- Deterministic routing remains the first rung; Jev tie-breaking is on by default only for an ambiguous lexical hint between two or three optional tools. Toggle it in `/settings`.
- Jev sees only the bounded current prompt and compact candidate lines — never schemas, history, or the full catalog. Token-In credentials are required; without them no Jev request is sent, and Selesai prompts you to add an account with `/tokenin add`. Unavailable, invalid, low-confidence, or `none` decisions leave deterministic behavior unchanged.
