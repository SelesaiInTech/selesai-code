...
- Routing is deterministic and local by default; only a uniquely matched tool is activated, and skills are never auto-loaded.
- Opt-in `capabilityGateway.routing.jev` can break an ambiguous lexical hint between two or three optional tools. Jev sees only the bounded current prompt and compact candidate lines — never schemas, history, or the full catalog — and an unavailable, invalid, low-confidence, or `none` decision changes nothing.
