...
test("native image errors abort paste without reading text or changing the editor", async () => {
	...
	const context = {
		...
		// Selesai delta: the fork surfaces a clipboard failure with a warning instead of
		// swallowing it (PRD 4/5 "a silent failure does not waste my time"). Upstream's
		// assertion intent below is unchanged. Recorded in GATES.pi-0861-sync.md.
		showWarning: vi.fn(),
	};
