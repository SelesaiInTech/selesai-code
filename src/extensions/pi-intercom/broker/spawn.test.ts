...
test("getBrokerLaunchSpec falls back to PATH node for a standalone Selesai executable on Windows", () => {
  ...
  try {
    const spec = getBrokerLaunchSpec(
      ...
      "win32",
      intercomDir,
      "C:/Program Files/Selesai/selesai.exe",
    );
    assert.equal(spec.kind, "windows-launcher");
...
test("getBrokerLaunchSpec falls back to PATH node for a standalone Selesai executable on non-Windows", () => {
  const spec = getBrokerLaunchSpec(
    ...
    "darwin",
    "/tmp/intercom",
    "/Applications/Selesai.app/Contents/MacOS/selesai",
  );
...
test("getBrokerSpawnOptions passes an absolute SELESAI_CODING_AGENT_DIR to the broker", () => {
  const options = getBrokerSpawnOptions("/repo", { SELESAI_CODING_AGENT_DIR: "relative-agent" });
  assert.equal(options.env.SELESAI_CODING_AGENT_DIR, path.resolve("relative-agent"));
});
