...
test("getAgentDirPath honors SELESAI_CODING_AGENT_DIR", () => {
  assert.equal(getAgentDirPath({ SELESAI_CODING_AGENT_DIR: "/tmp/selesai-agent" }), "/tmp/selesai-agent");
});

test("getAgentDirPath resolves relative SELESAI_CODING_AGENT_DIR values from the caller cwd", () => {
  const cwd = join(tmpdir(), "workspace", "project");
  assert.equal(
    getAgentDirPath({ SELESAI_CODING_AGENT_DIR: "relative-agent" }, cwd),
    join(cwd, "relative-agent"),
  );
});

test("getIntercomDirPath points at the intercom runtime directory under the agent dir", () => {
  assert.equal(getIntercomDirPath("/tmp/selesai-agent"), join("/tmp/selesai-agent", "intercom"));
});

test("getBrokerSocketPath uses named pipe on Windows", () => {
  const pipePath = getBrokerSocketPath("win32", "C:/Users/rcroh/.selesai/agent");
  assert.match(pipePath, /^\\\\\.\\pipe\\pi-intercom-/);
  assert.doesNotMatch(pipePath, /broker\.sock$/);
});

test("getBrokerSocketPath uses broker.sock under SELESAI_CODING_AGENT_DIR on non-Windows", () => {
  const socketPath = getBrokerSocketPath("linux", "/tmp/selesai-agent");
  assert.equal(socketPath, join("/tmp/selesai-agent", "intercom", "broker.sock"));
});
