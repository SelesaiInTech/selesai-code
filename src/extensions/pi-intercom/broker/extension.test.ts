...
async function startBroker(agentDir: string): Promise<ChildProcessWithoutNullStreams> {
  const broker = spawn(
    process.execPath,
    [getTsxCliPath(repoDir), path.join(repoDir, "broker", "broker.ts")],
    {
      cwd: repoDir,
      env: { ...process.env, SELESAI_CODING_AGENT_DIR: agentDir },
  ...
  const ready = new Promise<void>((resolve, reject) => {
    ...
    broker.once("exit", (code, signal) => {
      clearTimeout(timeout);
      reject(new Error(`Broker exited before startup (${code ?? signal}): ${broker.stderr.read()?.toString() ?? ""}`));
    });
  });
  await ready;
  return broker;
...
test("extension bus negotiates, routes, elects an owner, and persists state", { concurrency: false, timeout: 30_000 }, async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "ic-unit-"));
...
test("extension bus owners, publish, and state are scoped", { concurrency: false, timeout: 30_000 }, async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "ic-scope-"));
  const previousAgentDir = process.env.SELESAI_CODING_AGENT_DIR;
  process.env.SELESAI_CODING_AGENT_DIR = agentDir;
  const broker = await startBroker(agentDir);
  const clients: IntercomClient[] = [];
