...
process.env.SELESAI_CODING_AGENT_DIR = sharedHomeDir;
...
async function withChildOrchestratorEnv<T>(metadata: {
  ...
  if (metadata.orchestratorTarget !== undefined) process.env.SELESAI_SUBAGENT_ORCHESTRATOR_TARGET = metadata.orchestratorTarget;
  if (metadata.orchestratorSessionId !== undefined) process.env.SELESAI_SUBAGENT_ORCHESTRATOR_SESSION_ID = metadata.orchestratorSessionId;
  ...
  if (metadata.runId !== undefined) process.env.SELESAI_SUBAGENT_RUN_ID = metadata.runId;
  if (metadata.agent !== undefined) process.env.SELESAI_SUBAGENT_CHILD_AGENT = metadata.agent;
  if (metadata.index !== undefined) process.env.SELESAI_SUBAGENT_CHILD_INDEX = metadata.index;
  if (metadata.sessionName !== undefined) process.env.SELESAI_SUBAGENT_INTERCOM_SESSION_NAME = metadata.sessionName;
  if (metadata.supervisorChannelDir !== undefined) process.env.SELESAI_SUBAGENT_SUPERVISOR_CHANNEL_DIR = metadata.supervisorChannelDir;
  try {
    return await fn();
  } finally {
    for (const key of childEnvKeys) {
...
test("session_info_changed publishes Selesai name changes immediately", { concurrency: false }, async () => {
  const { planner, cleanup } = await setupClients();
  let sessionName = "event-name-before";
  const harness = createExtensionHarness(() => sessionName, { hasUI: true });
  ...
  try {
    await withChildOrchestratorEnv({ namePollMs: "60000" }, async () => {
      ...
      piIntercomExtension(harness.pi as never);
      await harness.emitLifecycle("session_start");
      await waitForSessionByName(planner, "event-name-before");
      sessionName = "event-name-after";
      await harness.emitLifecycle("session_info_changed", { name: sessionName });
      await waitForSessionByName(planner, "event-name-after");
    });
  } finally {
    await harness.emitLifecycle("session_shutdown");
    await cleanup();
...
test("turn_start re-registers when Selesai replaces the session context", { concurrency: false }, async () => {
  const { planner, cleanup } = await setupClients();
