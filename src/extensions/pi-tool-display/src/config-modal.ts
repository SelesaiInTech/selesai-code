...
function summarizeConfig(config: ToolDisplayConfig, capabilities: ToolDisplayCapabilities): string {
	...
	const parts = [
		...
		`boxedCalls=${toOnOff(config.boxedToolCalls)}`,
	];
...
function buildAdvancedNotes(
	...
	const notes = [
		...extra,
		"Manual JSON edits also expose boxedToolCalls, registerToolOverrides.*, expandedPreviewMaxLines, diffSplitMinWidth, diffCollapsedLines, diffIndicatorMode, and diffWordWrap.",
...
function buildInspectorSettings(
	...
	const items: InspectorSettingItem[] = [
		{
			...
			inspectorOptions: [
				...
				"boxed — ASCII-bordered box around each tool call and its output",
	...
	items.push(
		...
		{
			id: "boxedToolCalls",
			label: "Boxed tool calls",
			currentValue: toOnOff(config.boxedToolCalls),
			values: ["on", "off"],
			inspectorTitle: "Boxed Tool Calls",
			inspectorSummary: [
				"Draws an ASCII-bordered box (startup \"Tools\" style) around each tool call and its output.",
				"The preset command applies this automatically; this switch is the fine-grained control.",
			],
			inspectorOptions: [
				"on — each tool call and result is wrapped in a bordered frame",
				"off — default inline rendering without borders",
			],
			inspectorAdvanced: buildAdvancedNotes(config, capabilities, [
				"The /tool-display preset boxed command enables this; the other presets turn it off.",
			]),
			inspectorPath: configPath,
			searchTerms: ["box", "boxed", "border", "ascii", "frame", "tools"],
		},
		{
...
export function handleToolDisplayArgs(args: string, ctx: ExtensionCommandContext, controller: ToolDisplayConfigController): boolean {
	...
	if (normalized === "reset") {
		controller.setConfig(getToolDisplayPresetConfig("boxed"), ctx);
		ctx.ui.notify("Tool display preset reset to boxed (default).", "info");
		return true;
	}
...
export function registerToolDisplayCommand(pi: ExtensionAPI, controller: ToolDisplayConfigController): void {
	pi.registerCommand("tool-display", {
		description: "Configure tool output rendering (OpenCode/boxed styles)",
