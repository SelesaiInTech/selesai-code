...
import { boxToolCall, boxToolResult } from "./tool-call-box.js";
...
function maybeBoxCall(component: unknown, theme: RenderTheme, getConfig: ConfigGetter): unknown {
	return getConfig().boxedToolCalls && component
		? boxToolCall(component as unknown as Component & { invalidate?: () => void }, theme as never, true)
		: component;
}

function maybeBoxResult(component: unknown, theme: RenderTheme, getConfig: ConfigGetter): unknown {
	return getConfig().boxedToolCalls && component
		? boxToolResult(component as unknown as Component & { invalidate?: () => void }, theme as never, true)
		: component;
}

interface BoxedRendererOwnership {
	toolName: BuiltInToolOverrideName;
	getConfig: ConfigGetter;
}

function withBoxedRenderers(
	tool: RuntimeToolDefinition,
	ownership: BoxedRendererOwnership,
): RuntimeToolDefinition {
	return {
		...tool,
		// renderShell "self" is the upstream extension API: the host skips its colored
		// Box shell entirely and renders whatever this component returns. Combined with
		// the width-keyed frame cache in tool-call-box.ts, finalized tools cost one
		// array identity check per frame.
		renderShell: "self",
		renderCall(args, theme, context) {
			return maybeBoxCall(tool.renderCall?.(args, theme, context), theme as RenderTheme, ownership.getConfig);
		},
		renderResult(result, options, theme, context) {
			return maybeBoxResult(
...
function registerRuntimeTool(pi: ExtensionAPI, tool: RuntimeToolDefinition, ownership?: BoxedRendererOwnership): void {
	const registered = ownership ? withBoxedRenderers(tool, ownership) : tool;
	pi.registerTool(registered as unknown as ToolDefinition);
}
...
export function registerToolDisplayOverrides(
  ...
  const registerBuiltIn = (toolName: BuiltInToolOverrideName, tool: RuntimeToolDefinition): void => {
    registerRuntimeTool(pi, tool, { toolName, getConfig });
  };
