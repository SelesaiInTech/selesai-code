...
- **Four presets**: `opencode`, `balanced`, `verbose`, and `boxed`
...
/tool-display preset boxed            # Apply boxed preset (ASCII border per tool call)
```
...
| `boxed` (default) | hidden | hidden | hidden | opencode | 8 | 10 |

- **`boxed`** (default): ASCII-bordered box around each tool call and its output
...
| `boxedToolCalls` | boolean | `false` | Wrap each built-in tool call and its result in an ASCII-bordered box |
