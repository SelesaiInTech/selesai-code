...
### Added
- Added a `boxed` preset and `boxedToolCalls` config toggle that renders each built-in tool call and its result inside an ASCII-bordered box, matching the startup resource-list style.
