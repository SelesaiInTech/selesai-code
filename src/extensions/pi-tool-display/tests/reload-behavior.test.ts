...
test("12: config-store reloads config on fingerprint change between calls", () => {
  ...
  // The package config is gitignored and absent on a fresh checkout; use defaults
  // as a stand-in so the save/reload round-trip still has a real fixture.
  let originalConfigJson: string;
  let hadConfig = true;
  try {
    originalConfigJson = readFileSync(configUrl, "utf8");
  } catch {
    hadConfig = false;
    originalConfigJson = JSON.stringify(DEFAULT_TOOL_DISPLAY_CONFIG, null, 2);
  }

  try {
    const initialResult = loadToolDisplayConfig(configUrl.pathname);
    ...
    const saveResult = saveToolDisplayConfig(initialResult.config, configUrl.pathname);
    ...
    const afterSaveResult = loadToolDisplayConfig(configUrl.pathname);
