...
test("getToolDisplayPresetConfig returns correct config for boxed preset", () => {
	const config = getToolDisplayPresetConfig("boxed");

	assert.equal(config.boxedToolCalls, true);
	assert.equal(config.readOutputMode, "hidden");
	assert.equal(config.searchOutputMode, "hidden");
	assert.equal(config.bashOutputMode, "opencode");
});
...
test("detectToolDisplayPreset finds opencode when boxed calls are turned off", () => {
	const boxed = getToolDisplayPresetConfig("boxed");
	// The boxed preset is opencode + bordered calls, so disabling the border
	// resolves back to the opencode profile.
	assert.equal(detectToolDisplayPreset({ ...boxed, boxedToolCalls: false }), "opencode");
});
...
test("parseToolDisplayPreset parses 'boxed'", () => {
	assert.equal(parseToolDisplayPreset("boxed"), "boxed");
});
