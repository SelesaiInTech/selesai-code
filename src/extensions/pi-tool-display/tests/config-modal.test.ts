...
test("'show' argument notifies with config summary", async () => {
	...
	assert.ok(notifications[0]?.message.includes("preset=boxed"));
...
test("'reset' argument sets config to boxed default preset", async () => {
	const { api, getHandler } = createPiStub();
	...
	assert.equal(last.config!.boxedToolCalls, true);
	assert.match(notifications[0]?.message ?? "", /reset to boxed/i);
	assert.equal(notifications[0]?.level, "info");
});
