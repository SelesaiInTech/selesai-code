...
_Done when: users can customize behavior via `~/.selesai/agent/hermes-memory-config.json`._
- [x] Read config from `~/.selesai/agent/hermes-memory-config.json` — `src/config.ts`
...
1. Have the LLM save a memory and verify it appears in `~/.selesai/agent/memory/MEMORY.md`
