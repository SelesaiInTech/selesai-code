...
Hermes creates skills after complex tasks (5+ tool calls). Skills are SKILL.md files in `~/.hermes/skills/` with progressive disclosure. We adapt this for Pi's existing skill infrastructure at `~/.selesai/agent/skills/`.
...
- [x] Skill storage split by scope: global skills in `~/.selesai/agent/skills/<slug>/SKILL.md`, project skills in `~/.selesai/agent/projects-memory/<project>/skills/<slug>/SKILL.md` (discovered via `resources_discover`)
...
Project-scoped memory (`~/.selesai/agent/<project>/MEMORY.md`) was added in the feature branch that merged with v0.2.1. It works but needs cleanup, testing, and documentation.
...
- `~/.selesai/agent/memory/sessions.db` — single SQLite file
