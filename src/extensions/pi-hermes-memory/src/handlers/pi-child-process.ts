...
function resolvedInstalledPiCliPath(): string | undefined {
  try {
    const packageEntry = import.meta.resolve("@selesai/code");
...
function resolvedWindowsPiInvocation(
  ...
  for (const directory of pathEntries) {
    for (const executableName of ["selesai.exe", "selesai.com"]) {
    ...
    if (!existsSync(join(directory, "selesai.cmd")) && !existsSync(join(directory, "selesai.bat"))) {
      continue;
    }

    for (const cliPath of [
      join(directory, "node_modules", "@selesai", "code", "dist", "cli.js"),
      join(directory, "node_modules", "@selesai", "code", "cli.js"),
    ]) {
