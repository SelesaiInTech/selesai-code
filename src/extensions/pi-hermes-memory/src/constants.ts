...
SCOPE:
- 'global': transferable procedures that can be reused across repositories. Written to ~/.selesai/agent/pi-hermes-memory/skills/<slug>/SKILL.md, this extension's own directory, kept separate from skills the user installed themselves. Pi also loads its own ~/.selesai/agent/skills/ first, so a name already used there is rejected rather than silently shadowed.
- 'project': procedures tied to this repo's paths, scripts, architecture, deploy flow, or conventions. Written to ~/.selesai/agent/projects-memory/<project>/skills/<slug>/SKILL.md.
