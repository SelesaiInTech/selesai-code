...
const isolatedAgentDir = vi.hoisted(() => {
	const previous = process.env.SELESAI_CODING_AGENT_DIR;
	const path = `/tmp/pi-zentui-extension-compliance-${process.pid}`;
	const fs = process.getBuiltinModule("node:fs");
	fs.rmSync(path, { recursive: true, force: true });
	fs.mkdirSync(path, { recursive: true });
	process.env.SELESAI_CODING_AGENT_DIR = path;
...
function canonicalizeTestConfig(config: PolishedTuiConfig): PolishedTuiConfig {
	...
	return {
		...config,
		// Most compliance cases exercise Zentui-owned surfaces, so retain their explicit
		// pre-change fixture baseline rather than relying on bundled defaults.
		icons: { ...config.icons, mode: config.icons.mode === "ascii" ? "nerd" : config.icons.mode },
		components: {
			editor: {
				...editor,
				enabled: flatChanged("features") ? config.features.editor : true,
...
function complianceConfig(): PolishedTuiConfig {
	// Direct-install cases pin the legacy opt-in baseline explicitly so they keep
	// testing their own behavior instead of inheriting the bundled defaults.
	...
	config.components.footer.styles.starship.compactMaxLines = 2;
	return config;
}
...
function loadExtension(options: { thinkingLevel?: string; commands?: Map<string, unknown> } = {}) {
	...
	const persisted = existsSync(configPath)
		...
		: {};
	const components = (persisted.components as Record<string, unknown> | undefined) ?? {};
	// Compliance cases exercise opt-in surfaces; retain that intent explicitly instead
	// of inheriting the bundled native/disabled defaults.
	writeFileSync(
		configPath,
		`${JSON.stringify({
			...persisted,
			icons: { mode: "nerd", ...(persisted.icons as Record<string, unknown> | undefined) },
			components: {
				editor: (components.editor as Record<string, unknown> | undefined) ?? { enabled: true },
				userMessages: (components.userMessages as Record<string, unknown> | undefined) ?? {
					enabled: true,
					style: "framed",
				},
				workingLine: (components.workingLine as Record<string, unknown> | undefined) ?? {
					enabled: false,
				},
				footer: (components.footer as Record<string, unknown> | undefined) ?? {
					style: "starship",
				},
...
afterAll(() => {
	...
	if (isolatedAgentDir.previous === undefined) delete process.env.SELESAI_CODING_AGENT_DIR;
	else process.env.SELESAI_CODING_AGENT_DIR = isolatedAgentDir.previous;
});
...
`;
	...
	it("renders Zentui settings with mode-aware top and bottom borders", async () => {
		...
		const selectorTerminalConfig = complianceConfig();
