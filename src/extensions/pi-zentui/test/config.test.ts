...
describe("canonical config resolution", () => {
	it("provides complete canonical defaults and the established palette defaults", () => {
			...
			footer: {
				...
				styles: {
					starship: {
						...
						compactMaxLines: "unlimited",
						separator: "none",
						contextStyle: "text",
						contextThresholds: { warning: 70, error: 90 },
						pathDisplay: { mode: "basename", depth: 2 },
		...
		expect(config.icons.cacheHit).toBe("c");
	...
	it("ignores branch-only editorStyle/editorStyles and safely defaults closed style IDs", () => {
		...
		expect(config.components.userMessages.style).toBe("labeled");
...
describe("Phase 4 style migration", () => {
	...
	it("maps the released feature flag to copy-friendly Editor and messages", () => {
		...
		expect(regular.components.userMessages).toMatchObject({ style: "labeled", enabled: true });
...
describe("mergeConfig", () => {
	...
	it("defaults and normalizes responsive footer settings", () => {
		...
		for (const value of [0, 4, "2", null, false]) {
			expect(mergeConfig({ compactFooterMaxLines: value }).compactFooterMaxLines).toBe("unlimited");
		}
	...
	it("defaults separator style to none and accepts supported values", () => {
		expect(mergeConfig({}).separator).toBe("none");
		expect(defaultConfig.separator).toBe("none");
		for (const separator of ["pipe", "dot", "chevron", "none"] as const) {
			expect(mergeConfig({ separator }).separator).toBe(separator);
		}
	});

	it("falls back to none for invalid separator styles", () => {
		for (const separator of ["arrow", "", 123, null, true]) {
			expect(mergeConfig({ separator }).separator).toBe("none");
		}
	});
	...
	it("defaults editorModelLabel to name and accepts valid overrides", () => {
		expect(mergeConfig({}).editorModelLabel).toBe("name");
		...
		expect(mergeConfig({ editorModelLabel: "title" }).editorModelLabel).toBe("name");
	});
	...
	it("defaults and normalizes editor border color mode", () => {
		expect(defaultConfig.editorBorderColorMode).toBe("adaptive");
		expect(mergeConfig({}).editorBorderColorMode).toBe("adaptive");
		...
		for (const value of ["dynamic", "", 1, null, true]) {
			expect(mergeConfig({ editorBorderColorMode: value }).editorBorderColorMode).toBe("adaptive");
		}
	});
	...
	it("defaults pathDisplay and accepts mode/depth overrides", () => {
		expect(mergeConfig({}).pathDisplay).toEqual({ mode: "basename", depth: 2 });
		...
		).toEqual({ mode: "repository", depth: 2 });		expect(mergeConfig({ pathDisplay: { mode: "fish", depth: -3 } }).pathDisplay).toEqual({
			mode: "basename",
			depth: 2,
		});
...
`,
	...
	it("defaults icon mode to ascii and accepts nerd/ascii", () => {
		expect(mergeConfig({}).icons.mode).toBe("ascii");
		expect(mergeConfig({ icons: { mode: "ascii" } }).icons.mode).toBe("ascii");
		expect(mergeConfig({ icons: { mode: "nerd" } }).icons.mode).toBe("nerd");
		expect(mergeConfig({ icons: { mode: "emoji" } }).icons.mode).toBe("ascii");
	...
	it("accepts valid UI feature preferences and ignores invalid values", () => {
		...
		).toEqual({ editor: false, statusLine: false, viewportIndicators: false });
	});
...
describe("Opencode and Footer follow-up migration", () => {
	...
	it("resolves Footer style from valid style, enabled, released statusLine, then default", () => {
		...
		expect(mergeConfig({}).components.footer.style).toBe("native");
