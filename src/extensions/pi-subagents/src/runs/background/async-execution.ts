...
import { SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV, resolveChildCwd } from "../../shared/utils.ts";
...
const selesaiPackageRoot = resolveSelesaiPackageRoot();
...
function resolveJitiCliPath(): string | undefined {
	const candidates: Array<() => string | undefined> = [
		...
		() => selesaiPackageRoot
			? createRequire(path.join(selesaiPackageRoot, "package.json")).resolve("jiti/package.json")
			: undefined,
		() => {
		...
		() => selesaiPackageRoot ? path.join(selesaiPackageRoot, "node_modules", "jiti", "package.json") : undefined,
	];
	for (const candidate of candidates) {
		try {
...
function spawnRunner(cfg: object, suffix: string, cwd: string): { pid?: number; error?: string } {
	...
	try {
		...
		const proc = spawn(nodeCommand, [jitiCliPath, runner, cfgPath], {
			...
			env: {
				...process.env,
				...(selesaiPackageRoot ? { [SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV]: selesaiPackageRoot } : {}),
			},
		});
		closeFd(stdoutFd);
		closeFd(stderrFd);
			...
			try {
				writeNestedEvent(inheritedNestedRoute, {
					...
					child: {
						...
						asyncDir,
						pid: spawnResult.pid,
						ownerIntercomTarget: process.env.SELESAI_SUBAGENT_INTERCOM_SESSION_NAME,
