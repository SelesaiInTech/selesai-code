...
name: pi-subagents
description: |
  Delegate work to builtin or custom subagents with single-agent, scripted,
  compatibility-chain, async, forked-context, and intercom-coordinated workflows. Use
...
| Task | Read |
| --- | --- |
| Decide whether to explorer, choose agents, compare tool versus slash commands, apply prompt techniques, or understand builtin roles | `references/prompting-and-roles.md` |
| Run single, scripted, async, scheduled, mission-backed, forked, worktree, watchdog, clarify, commentator, or intercom-coordinated workflows | `references/execution-controls.md` |
...
Routing rule: keep tiny targeted reads and simple answers with the parent. For broad local investigation, external research, or mutation work, call `{ action: "list" }`, choose an executable entry from its runtime metadata, then explorer. Treat list output—not hardcoded role names—as the agent-selection authority.
