...
description: |
  ...
  for commentatory review, implementation handoffs, and multi-step tasks where a
...
- **Subagent control**: watch needs-attention signals and soft-interrupt only when a builderd run is genuinely blocked
...
- Skip proactive skill subagents for tiny questions, direct commands, highly private requests, or when the user asks not to builder.
...
| `commentator` | Review-and-fix specialist | inherits default | Can edit/fix reviewed code |
...
| `builder` | Lightweight generic builder | inherits default | No fixed output; generic builderd work |
| `commentator` | Decision-consistency commentatory review | inherits default | Advisory review, intercom coordination |
| `commentator` | Claude Code-compatible alias for `commentator` | inherits default | Same commentatory role as `commentator` |
...
Settings locations:
- User scope: `~/.selesai/agent/settings.json`
- Project scope: `.selesai/settings.json`
...
subagent({
  tasks: [
    { agent: "explorer", task: "Explore the auth module" },
    { agent: "commentator", task: "Review the API client" }
  ]
})
```
...
subagent({
  tasks: [
    { agent: "explorer", task: "Map auth", output: "auth-context.md", progress: true },
    { agent: "researcher", task: "Research OAuth best practices", output: "oauth-research.md" },
    { agent: "commentator", task: "Review auth tests", model: "anthropic/claude-sonnet-4" }
  ],
  concurrency: 3
})
```
...
subagent({
  chain: [
    { agent: "explorer", task: "Map the auth flow and summarize key files" },
    { agent: "architect", task: "Create an implementation plan from {previous}" },
    { agent: "builder", task: "Implement the approved plan based on {previous}" }
  ]
})
```
...
Prefer async mode for every subagent launch. Set `async: true` no matter the task unless there is a specific reason to opt into a foreground/blocking run. This applies to explorers, researchers, builders, commentators, validators, commentator checks, one-off builders, chains, and parallel groups. Keep the write path single-threaded even when the run is async.

Async does not mean parallel writes. Do not edit the same active worktree while an async builder is changing it. Parent-side overlap should be reading, validation prep, synthesis, command planning, or review of unaffected context unless the writer is isolated in a separate worktree.
...
Inspect async runs with `subagent({ action: "status", id: "..." })` or `subagent({ action: "status" })` for active runs. Use `subagent({ action: "status", view: "fleet" })` when supervising several active foreground/background runs and `subagent({ action: "status", id: "...", view: "transcript", index: 0 })` when you need the latest child output without digging through artifacts. If a builderd fanout child launches nested runs, the parent status view shows them as a tree and you can target a nested run directly with its nested id.

Use `steer` for top-level live async guidance and `resume` after a builderd run pauses or finishes. Routed nested runs retain their existing non-destructive live follow-up path:
...
subagent({ action: "resume", id: "run-id", index: 1, message: "Continue commentator 2." })
subagent({ action: "resume", id: "nested-run-id", message: "Continue this nested commentator." })
```
...
Scheduled runs defer a subagent launch until a future time. They are opt-in and require `{ "scheduledRuns": { "enabled": true } }` in `~/.selesai/agent/extensions/subagent/config.json`. Only schedule explicit delayed runs the user asked for; do not schedule runs speculatively.

```typescript
// Launch a commentator in 30 minutes
subagent({ action: "schedule", agent: "commentator", task: "Review the diff for correctness issues.", schedule: "+30m", scheduleName: "evening review" })

// Schedule a parallel fanout
subagent({ action: "schedule", tasks: [{ agent: "explorer", task: "Map the auth module" }, { agent: "explorer", task: "Map the billing module" }], schedule: "+1h" })
...
A soft interrupt cancels the current child turn and leaves the run paused. It does not mean the builderd task succeeded or failed. Bare `interrupt` does not target hidden nested descendants; use the explicit nested id. After an interrupt, decide the next explicit action: resume with clearer instructions, replace the task, ask the user, or stop the workflow.
...
subagent({
  tasks: [
    { agent: "builder", task: "Implement feature A" },
    { agent: "builder", task: "Implement feature B" }
  ],
  worktree: true
})
```
...
removed. If you want one writer thread and several commentatory agents, prefer a
...
subagent({ action: "eject", agent: "commentator" })
subagent({ action: "eject", agent: "commentator", agentScope: "project" })

// Hide an agent from runtime discovery without deleting it (reversible).
subagent({ action: "disable", agent: "commentator" })
subagent({ action: "enable", agent: "commentator", agentScope: "project" })

// Delete the scope's custom agent file and/or settings override, restoring the bundled default.
subagent({ action: "reset", agent: "commentator" })
```
...
`/parallel-cleanup autofix` synthesize commentator feedback and then apply only the
...
Additional user prompt templates can builder into `pi-subagents` through the native `/prompt-workflow` and `/chain-prompts` commands. This is useful when a slash command should always run through a particular agent or with forked context. Prompt frontmatter can set `subagent`, `model`, `skill`, `cwd`, `worktree`, `fresh`, `fork`, or `inheritContext` for the native adapter.
...
Use subagent control proactively when a builderd run emits `needs_attention`, or when a human asks you to regain control. Do not interrupt just because a child has briefly produced no output. Silence can be normal during long tool calls, test runs, or model reasoning.
