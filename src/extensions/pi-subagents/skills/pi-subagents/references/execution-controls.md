...
**Workflow modes as saved chains.** The bundled `workflow` extension ships its four mode shapes (task, prototype, quicktype, loop) as pi-subagents chains launched via `/workflow-*` — the same slash seam as `/run-chain`. Phases are chain steps, human gates are `checkpoint` steps (approve via `approve-checkpoint`, reject via `reject-checkpoint`), and verified acceptance is the chain `acceptance` gate. Durable process state is the auto-created mission per launch; resumability comes from retained children (`children.list` + `runs.run({ resume })`).
