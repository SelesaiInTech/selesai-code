...
import type { AgentSessionEvent, SessionBeforeCompactEvent } from "@selesai/code";
...
for (const host of ["foreground", "runner"] as const) {
	for (const [systemPromptMode, structured] of [["append", false], ["replace", false], ["append", true], ["replace", true]] as const) {
		it(`exact acceptance survives actual SDK split-turn compaction: ${host}/${systemPromptMode}/${structured ? "structured" : "fenced"}`, { skip: !sdkRoot && "Set SELESAI_SUBAGENTS_NATIVE_SDK to the isolated 0.85.1 SDK root" }, async () => {
			...
			const oldDir = process.env.SELESAI_CODING_AGENT_DIR;
			...
			try {
				...
				if (oldDir === undefined) delete process.env.SELESAI_CODING_AGENT_DIR; else process.env.SELESAI_CODING_AGENT_DIR = oldDir;
