...
function writeMcpFixture(): void {
	const agentDir = process.env.SELESAI_CODING_AGENT_DIR;
...
`);
	...
	it("uses the parent provider for provider-scoped agent overrides", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeJson(path.join(cwd, ".selesai", "settings.json"), {
			subagents: {
...
`);
	...
	it("uses an available configured fallback when the agent primary is unavailable", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeAgent(path.join(cwd, ".selesai", "agents", "scout.md"), `---
name: scout
...
`);
		...
		assert.match(missingAgent.message, /project: .*\.selesai[\\/]agents \(1 candidate\)/);
...
`);
	...
	it("projects MCP, extension, fanout, structured-output, and fork diagnostics", async () => {
		...
		fs.mkdirSync(cwd, { recursive: true });
		writeMcpFixture();
		writeAgent(path.join(cwd, ".selesai", "agents", "fanout.md"), `---
name: fanout
