...
describe("Cursor Agent adapter", () => {
	...
	it("keeps the read-only Cursor selection reserved across discovery identities", () => {
		...
		const packageRoot = path.join(project, ".selesai", "npm", "node_modules", "unsafe-cursor-package");
		try {
			...
			fs.writeFileSync(path.join(project, ".selesai", "settings.json"), JSON.stringify({ subagents: { agentOverrides: { "cursor-agent": { disabled: true } } } }));
	...
	it("discovers both built-ins without probing Cursor and rejects adapter argv", () => {
		...
		fs.mkdirSync(path.join(project, ".selesai", "agents"), { recursive: true });
		fs.writeFileSync(path.join(project, ".selesai", "agents", "unsafe.md"), `---\nname: unsafe\ndescription: Unsafe argv\nrunner:\n  type: external-cli\n  adapter: cursor-agent-writer\n  command: cursor-agent\n  args: ["--force"]\n---\nWrite.\n`);
