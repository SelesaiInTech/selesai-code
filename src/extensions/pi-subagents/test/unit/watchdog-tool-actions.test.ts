...
describe("watchdog tool actions", () => {
	beforeEach(() => {
		...
		process.env.SELESAI_CODING_AGENT_DIR = path.join(tempHome, ".selesai", "agent");
	});
	...
	it("configures the recommended model for the current session without writing settings", () => {
		...
		assert.equal(fs.existsSync(path.join(tempHome, ".selesai", "agent", "settings.json")), false);
	...
	it("rejects transport-permitted true thinking without changing session or settings", () => {
		...
		assert.equal(fs.existsSync(path.join(tempHome, ".selesai", "agent", "settings.json")), false);
		assert.equal(fs.existsSync(path.join(tempProject, ".selesai", "settings.json")), false);
	});
	...
	it("persists the recommended model when user scope is explicit", () => {
		...
		assert.equal(result.isError, undefined);
		const settings = JSON.parse(fs.readFileSync(path.join(tempHome, ".selesai", "agent", "settings.json"), "utf-8"));
