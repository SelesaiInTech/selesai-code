...
describe("SELESAI_CODING_AGENT_DIR runtime paths", () => {
	...
	it("resolves the agent dir dynamically and loads extension config from it", () => {
		...
		assert.equal(resolveConfigDirName({ CONFIG_DIR_NAME: "" }), ".selesai");
		assert.equal(getConfigDirName(), ".selesai");
		assert.equal(getProjectConfigDir(cwd), path.join(cwd, ".selesai"));
		assert.equal(getAgentDir(), agentDir);

		process.env.SELESAI_CODING_AGENT_DIR = "~";
		assert.equal(getAgentDir(), tempHome);

		process.env.SELESAI_CODING_AGENT_DIR = "~/custom-agent-dir";
		assert.equal(getAgentDir(), path.join(tempHome, "custom-agent-dir"));

		delete process.env.SELESAI_CODING_AGENT_DIR;
		assert.equal(getAgentDir(), path.join(tempHome, ".selesai", "agent"));
...
`);
		const nestedBackupAgentPath = path.join(userAgentsDir, ".selesai", "agent", "sync-backups", "20260712-163714", "agents", "stale.md");
