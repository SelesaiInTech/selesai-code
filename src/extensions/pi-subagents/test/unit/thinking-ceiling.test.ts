...
describe("thinking ceilings", () => {
	...
	it("loads maxThinking with project precedence and attaches it to discovered agents", () => {
		writeJson(path.join(home, ".selesai", "agent", "settings.json"), { subagents: { maxThinking: "max" } });
		fs.mkdirSync(path.join(project, ".selesai"), { recursive: true });
		writeJson(path.join(project, ".selesai", "settings.json"), { subagents: { maxThinking: "xhigh" } });
