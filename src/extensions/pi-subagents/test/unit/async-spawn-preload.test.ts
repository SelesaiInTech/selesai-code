...
test("executeAsyncSingle preloads supplemental server aliases before jiti, but not for complete hosts", async (t) => {
	...
	function writeHostPackage(pkg: string) {
		const dir = pkg === "@selesai/code" ? host : path.join(host, "node_modules", pkg);
	...
	try {
		...
		process.argv[1] = expectedAliases["@selesai/code"];
		...
		for (const scenario of ["supplemental", "complete", "stable", "missing-stable"]) {
			...
			if (scenario === "stable") {
				fs.writeFileSync(path.join(host, "package.json"), JSON.stringify({ name: "@selesai/code", version: "0.85.1", exports: { ".": "./index.mjs" } }));
