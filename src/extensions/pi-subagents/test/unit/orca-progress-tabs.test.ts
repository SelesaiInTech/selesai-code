...
test("resolveOrcaCommand only returns executable commands", () => {
	...
	assert.equal(resolveOrcaCommand({ PATH: "", SELESAI_SUBAGENT_ORCA_BINARY: path.join(dir, "missing") }), undefined);
});
...
test("an unavailable Orca command leaves native execution untouched", () => {
	...
	assert.equal(createOrcaProgressTab({
		...
		index: 0,
		config: { enabled: true },
		env: { PATH: "", SELESAI_SUBAGENT_ORCA_BINARY: path.join(dir, "missing") },
	}), undefined);
});
