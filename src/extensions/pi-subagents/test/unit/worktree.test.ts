...
`, "utf-8");
	...
	it("rejects worktree base directories inside Pi extensions", async () => {
		...
		const extensionsDir = path.join(tempHome, ".selesai", "agent", "extensions");
		try {
	...
	it("rejects a final project directory inside Pi extensions", async () => {
		...
		const agentDir = path.join(tempHome, ".selesai", "agent");
	...
	it("uses SELESAI_SUBAGENTS_WORKTREE_DIR when no base directory is configured", async () => {
		const repoDir = createRepo("pi-worktree-env-base-dir-");
		const previous = process.env.SELESAI_SUBAGENTS_WORKTREE_DIR;
		const baseDir = path.join(os.tmpdir(), `pi-worktree-env-base-${Date.now().toString(36)}`);
		let setup: WorktreeSetup | undefined;
		try {
			process.env.SELESAI_SUBAGENTS_WORKTREE_DIR = baseDir;
			...
			if (previous === undefined) {
				delete process.env.SELESAI_SUBAGENTS_WORKTREE_DIR;
			} else {
				process.env.SELESAI_SUBAGENTS_WORKTREE_DIR = previous;
			}
			cleanupRepo(repoDir, baseDir);
		}
	});
