...
beforeEach(() => {
	savedDepth = process.env.SELESAI_SUBAGENT_DEPTH;
	savedMaxDepth = process.env.SELESAI_SUBAGENT_MAX_DEPTH;
	savedMaxSpawns = process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION;
});

afterEach(() => {
	if (savedDepth === undefined) delete process.env.SELESAI_SUBAGENT_DEPTH;
	else process.env.SELESAI_SUBAGENT_DEPTH = savedDepth;
	if (savedMaxDepth === undefined) delete process.env.SELESAI_SUBAGENT_MAX_DEPTH;
	else process.env.SELESAI_SUBAGENT_MAX_DEPTH = savedMaxDepth;
	if (savedMaxSpawns === undefined) delete process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION;
	else process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION = savedMaxSpawns;
});
...
describe("resolveMaxSubagentSpawnsPerSession", () => {
	it("uses positive env values as opt-in caps", () => {
		process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION = "5";
		assert.equal(resolveMaxSubagentSpawnsPerSession(1), 5);
	});

	it("falls back to a positive config cap when env is absent", () => {
		delete process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION;
	...
	it("ignores invalid env values and falls back to config", () => {
		for (const value of ["garbage", "-1", "1.5"]) {
			process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION = value;
			assert.equal(resolveMaxSubagentSpawnsPerSession(7), 7);
	...
	it("treats zero as an explicit unlimited override", () => {
		process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_SESSION = "0";
...
describe("resolveCurrentMaxSubagentDepth", () => {
	it("uses env when present", () => {
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "5";
...
describe("checkSubagentDepth", () => {
	it("not blocked at depth=0, max=2", () => {
		...
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "2";
	...
	it("blocked at depth=2, max=2", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "2";
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "2";
		const result = checkSubagentDepth();
		assert.equal(result.blocked, true);
	...
	it("blocked at depth=3, max=2", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "3";
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "2";
		assert.equal(checkSubagentDepth().blocked, true);
	});
	...
	it("blocked at depth=0, max=0 (disables subagent entirely)", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "0";
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "0";
		assert.equal(checkSubagentDepth().blocked, true);
	});
	...
	it("not blocked when depth is invalid (NaN)", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "garbage";
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "2";
		assert.equal(checkSubagentDepth().blocked, false);
	});
});
...
describe("getSubagentDepthEnv", () => {
	it("increments from depth=0", () => {
		...
		assert.equal(env.SELESAI_SUBAGENT_DEPTH, "1");
		assert.equal(env.SELESAI_SUBAGENT_MAX_DEPTH, "2");
	});
	...
	it("increments from depth=1", () => {
		...
		delete process.env.SELESAI_SUBAGENT_MAX_DEPTH;
		const env = getSubagentDepthEnv();
		assert.equal(env.SELESAI_SUBAGENT_DEPTH, "2");
		assert.equal(env.SELESAI_SUBAGENT_MAX_DEPTH, "2");
	});
	...
	it("uses provided max depth override", () => {
		...
		assert.equal(env.SELESAI_SUBAGENT_DEPTH, "1");
		assert.equal(env.SELESAI_SUBAGENT_MAX_DEPTH, "1");
	});

	it("respects inherited SELESAI_SUBAGENT_MAX_DEPTH when override is absent", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "0";
		process.env.SELESAI_SUBAGENT_MAX_DEPTH = "5";
		const env = getSubagentDepthEnv();
		assert.equal(env.SELESAI_SUBAGENT_DEPTH, "1");
		assert.equal(env.SELESAI_SUBAGENT_MAX_DEPTH, "5");
	});
	...
	it("falls back to depth=1 when env var is invalid (NaN)", () => {
		process.env.SELESAI_SUBAGENT_DEPTH = "not-a-number";
		delete process.env.SELESAI_SUBAGENT_MAX_DEPTH;
		const env = getSubagentDepthEnv();
		assert.equal(env.SELESAI_SUBAGENT_DEPTH, "1");
	});
