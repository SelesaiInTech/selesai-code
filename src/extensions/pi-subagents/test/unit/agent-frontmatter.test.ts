...
function writeLinkedWorktreePackage(repo: string): { packageAgentName: string; packageAgentPath: string; settingsPath: string } {
	...
	const packageRoot = path.join(repo, ".selesai", "vendor", "workflow");
	const packageAgentPath = path.join(packageRoot, "agents", "reviewer.md");
	const settingsPath = path.join(repo, ".selesai", "settings.json");
...
describe("agent definition directory inspection", () => {
	...
	it("reports cached builtin and configured project definition paths without synthesizing a root", () => withTempHome(() => {
		...
		writeJson(path.join(project, ".selesai", "settings.json"), { subagents: { projectRootResolution: "nearest" } });
		const discovered = discoverAgents(project, "project");
		assert.ok(discovered.directories.some((entry) => entry.source === "builtin"));
		assert.deepEqual(discovered.directories.filter((entry) => entry.source === "project").map((entry) => entry.path), [path.join(project, ".agents"), path.join(project, ".selesai", "agents")]);
	}));
});
...
Review carefully.`.replace(" description:", "description:"));
	...
	it("keeps external-cli stop code-owned while refusing capability widening", () => withTempHome(() => {
		...
		tempDirs.push(project);
		const agentPath = path.join(project, ".selesai", "agents", "external.md");
	...
	it("parses and serializes an external-job runner", () => withTempHome(() => {
		...
		tempDirs.push(project);
		writeAgent(path.join(project, ".selesai", "agents", "gpt-pro.md"), `---
name: gpt-pro
...
Review carefully.`);
	...
	it("keeps valid agents executable when another agent is malformed", () => withTempHome(() => {
		...
		tempDirs.push(project);
		writeAgent(path.join(project, ".selesai", "agents", "broken.md"), "---\nname: broken\ndescription: Broken\nrunner:\n  type: unknown\n---\nBody");
		writeAgent(path.join(project, ".selesai", "agents", "working.md"), "---\nname: working\ndescription: Working\n---\nBody");
	...
	it("keeps a lower-priority agent available when a project override is malformed", () => withTempHome(() => {
		...
		tempDirs.push(project);
		writeAgent(path.join(project, ".selesai", "agents", "reviewer.md"), "---\nname: reviewer\ndescription: Broken reviewer\nrunner:\n  type: unknown\n---\nBody");
	...
	it("records the runtime name for malformed packaged agents", () => withTempHome(() => {
		...
		tempDirs.push(project);
		writeAgent(path.join(project, ".selesai", "agents", "code-analysis.zeta-worker.md"), "---\nname: zeta-worker\npackage: code-analysis\ndescription: Broken packaged worker\nrunner:\n  type: unknown\n---\nBody");
...
`);
		...
		assert.deepEqual(worker?.extensions, [path.join(dir, ".selesai", "agents", "extension-one.ts"), path.join(dir, ".selesai", "agents", "extension-two.ts")]);
		assert.deepEqual(worker?.subagentOnlyExtensions, [path.join(dir, ".selesai", "agents", "child-only.ts"), path.join(dir, ".selesai", "agents", "child-helper.ts")]);
	});
...
`);
	...
	it("discovers and serializes excludeTools without validating names", () => {
		...
		tempDirs.push(dir);
		const agentPath = path.join(dir, ".selesai", "agents", "worker.md");
		writeAgent(agentPath, `---
name: worker
description: Worker
excludeTools:
...
`);
	...
	it("loads bare HTTPS git packages from project Pi settings", () => withTempHome(() => {
		...
		tempDirs.push(dir);
		const packageRoot = path.join(dir, ".selesai", "git", "github.com", "user", "repo");
		writeJson(path.join(dir, ".selesai", "settings.json"), {
...
`);
	...
	it("keeps git-root discovery stable when a nested linked worktree creates incidental .selesai state", () => withTempHome(() => {
		...
		fs.mkdirSync(path.join(worktree, ".selesai", "todos"), { recursive: true });
	...
	it("lets a linked worktree opt back into nearest-root discovery", () => withTempHome(() => {
		...
		const worktreeSettingsPath = path.join(worktree, ".selesai", "settings.json");
