...
describe("child runtime config", () => {
	...
	it("hooks read the config object", async () => {
		...
		assert.deepEqual(diagnostics, [{ agent: "config-agent", required: ["read", "fixture_search"], available: ["read", "bg_wait", "subagent_wait", "structured_output"], missing: ["fixture_search"] }]);
