...
describe("subagent capability ceiling", () => {
	...
	it("combines inherited process policy with exact-session registrations", () => {
		const previous = process.env.SELESAI_SUBAGENT_CAPABILITY_CEILING_V1;
		...
		try {
			process.env.SELESAI_SUBAGENT_CAPABILITY_CEILING_V1 = encodeSubagentCapabilityCeiling({ version: 1, allowedTools: ["read", "write"], denyExtensions: true, sources: ["ancestor"] });
			...
			handle.dispose();
			if (previous === undefined) delete process.env.SELESAI_SUBAGENT_CAPABILITY_CEILING_V1;
			else process.env.SELESAI_SUBAGENT_CAPABILITY_CEILING_V1 = previous;
		}
	});
});