...
it("plans only from a real settled receipt: ordered siblings, vetoes, one-cap transitions and revocation", { skip: !sdkRoot && "Set SELESAI_SUBAGENTS_NATIVE_SDK" }, async () => {
	...
	const priorDir = process.env.SELESAI_CODING_AGENT_DIR, priorFetch = globalThis.fetch;
	...
	try {
		...
		if (priorDir === undefined) delete process.env.SELESAI_CODING_AGENT_DIR; else process.env.SELESAI_CODING_AGENT_DIR = priorDir;
