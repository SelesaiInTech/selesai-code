...
const originalEnv = {
	...
	SELESAI_SUBAGENT_PARENT_EVENT_SINK: process.env.SELESAI_SUBAGENT_PARENT_EVENT_SINK,
	SELESAI_SUBAGENT_PARENT_CONTROL_INBOX:
		process.env.SELESAI_SUBAGENT_PARENT_CONTROL_INBOX,
	SELESAI_SUBAGENT_PARENT_ROOT_RUN_ID: process.env.SELESAI_SUBAGENT_PARENT_ROOT_RUN_ID,
	SELESAI_SUBAGENT_PARENT_RUN_ID: process.env.SELESAI_SUBAGENT_PARENT_RUN_ID,
	SELESAI_SUBAGENT_PARENT_CHILD_INDEX: process.env.SELESAI_SUBAGENT_PARENT_CHILD_INDEX,
	SELESAI_SUBAGENT_PARENT_DEPTH: process.env.SELESAI_SUBAGENT_PARENT_DEPTH,
	SELESAI_SUBAGENT_PARENT_PATH: process.env.SELESAI_SUBAGENT_PARENT_PATH,
	SELESAI_SUBAGENT_PARENT_CAPABILITY_TOKEN:
		process.env.SELESAI_SUBAGENT_PARENT_CAPABILITY_TOKEN,
	...
	[SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV]:
		process.env[SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV],
	...
	SELESAI_SUBAGENT_TASK_DELIVERY: process.env.SELESAI_SUBAGENT_TASK_DELIVERY,
};
...
describe("buildPiArgs session wiring", () => {
	...
	it("emits explicit parent session env for permission forwarding", () => {
		process.env.SELESAI_SUBAGENT_PARENT_SESSION = "inherited-parent";
...
describe("buildPiArgs task delivery", () => {
	...
	it("delivers short tasks via file when SELESAI_SUBAGENT_TASK_DELIVERY=file", () => {
		process.env.SELESAI_SUBAGENT_TASK_DELIVERY = "file";
		const { args } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
	...
	it("lets the per-launch taskDelivery override beat the env setting", () => {
		delete process.env.SELESAI_SUBAGENT_TASK_DELIVERY;
		const { args } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			taskDelivery: "file",
...
describe("buildPiArgs system prompt mode wiring", () => {
	...
	it("injects the subagent prompt runtime extension and env flags", () => {
		...
		assert.equal(env.SELESAI_SUBAGENT_CHILD, "1");
		assert.equal(env.SELESAI_SUBAGENT_INHERIT_PROJECT_CONTEXT, "0");
		assert.equal(env.SELESAI_SUBAGENT_INHERIT_GLOBAL_CONTEXT, "0");
		assert.equal(env.SELESAI_SUBAGENT_INHERIT_SKILLS, "1");
	});
	...
	it("propagates the global context inheritance flag through env", () => {
		...
		assert.equal(env.SELESAI_SUBAGENT_INHERIT_PROJECT_CONTEXT, "1");
		assert.equal(env.SELESAI_SUBAGENT_INHERIT_GLOBAL_CONTEXT, "1");
	});
	...
	it("passes child intercom and orchestrator metadata through env", () => {
		...
		assert.equal(
			env.SELESAI_SUBAGENT_INTERCOM_SESSION_NAME,
	...
	it("forwards the Pi package root to child processes for host peer resolution", () => {
		process.env[SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV] = "/opt/pi-coding-agent";
		const { env } = buildPiArgs({
			baseArgs: ["-p"],
			task: "hello",
			sessionEnabled: false,
		...
		assert.equal(env[SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV], "/opt/pi-coding-agent");
	});
	...
	it("resolves project MCP config from the child cwd and expands SELESAI_CODING_AGENT_DIR", () => {
		const fixture = createMcpFixture();
		process.env.SELESAI_CODING_AGENT_DIR = "~/.selesai/agent";
		process.chdir(fixture.root);
		fs.mkdirSync(path.join(fixture.projectDir, ".selesai"));
		writeMcpFixture(fixture, {
	...
	it("resolves direct MCP tools from the git-root package when child cwd has an incidental .selesai directory", () => {
		...
		fs.mkdirSync(path.join(nestedCwd, ".selesai"));
		fs.mkdirSync(path.join(fixture.projectDir, ".git"));
		writeJson(path.join(fixture.projectDir, ".selesai", "settings.json"), {
