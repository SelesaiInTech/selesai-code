...
const previousEnv = process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN;

afterEach(() => {
	for (const directory of directories.splice(0)) fs.rmSync(directory, { recursive: true, force: true });
	for (const directory of externalDirectories.splice(0)) fs.rmSync(directory, { recursive: true, force: true });
	if (previousEnv === undefined) delete process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN;
	else process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN = previousEnv;
});
...
describe("run fan-out budget", () => {
	it("resolves environment over config and falls back to 64", () => {
		delete process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN;
		...
		process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN = "7";
