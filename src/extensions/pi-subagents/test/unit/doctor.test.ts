...
describe("buildDoctorReport", () => {
			...
			const report = buildDoctorReport({
				...
				deps: {
					...
					discoverAgentsAll: () => ({
						...
						agentDiagnostics: [{ source: "project", filePath: path.join(root, ".selesai", "agents", "broken.md"), name: "broken", error: "invalid runner" }],
						chains: [makeChain("user-flow", "user"), makeChain("project-flow", "project")],
						userDir: path.join(root, "home", ".agents"),
						projectDir: path.join(root, ".selesai", "agents"),
						userChainDir: path.join(root, "home", ".selesai", "agent", "chains"),
						projectChainDir: path.join(root, ".selesai", "chains"),
						userSettingsPath: path.join(root, "home", ".selesai", "agent", "settings.json"),
						projectSettingsPath: path.join(root, ".selesai", "settings.json"),
					}),
	...
	it("reports the effective source when the run fan-out environment value is invalid", () => {
		...
		const previous = process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN;
		try {
			process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN = "invalid";
			const report = buildDoctorReport({
				cwd: root,
			...
			if (previous === undefined) delete process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN;
			else process.env.SELESAI_SUBAGENT_MAX_SPAWNS_PER_RUN = previous;
			fs.rmSync(root, { recursive: true, force: true });
		}
	});
