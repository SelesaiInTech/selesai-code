...
import { SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV } from "../../src/runs/shared/extension-bindings.ts";
...
describe("external CLI runner", () => {
	it("clears ambient extension bindings from the external process", async () => {
		const dir = tempDir();
		const previous = process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV];
		process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV] = '{"shepherd.dispatch/1":{"role":"coder"}}';
		try {
			const result = await runExternalCli({
				command: process.execPath,
				args: ["-e", `process.stdout.write(process.env.${SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV} ?? "unset")`],
				cwd: dir,
				prompt: "x",
				asyncDir: dir,
				stepIndex: 0,
			...
			if (previous === undefined) delete process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV];
			else process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV] = previous;
		}
	});
	...
	it("passes only adapter-allowed environment keys", async () => {
		...
		const previousBinding = process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV];
		process.env.UNRELATED_SECRET = "hidden";
		process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV] = "hidden-binding";
		try {
			const result = await runExternalCli({
				command: process.execPath,
				args: ["-e", `process.stdout.write(JSON.stringify({allowed:process.env.ALLOWED_VALUE,secret:process.env.UNRELATED_SECRET,binding:process.env.${SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV}}))`],
				cwd: dir, prompt: "x", asyncDir: dir, stepIndex: 13,
				environment: { allowlist: ["ALLOWED_VALUE", SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV], values: { ALLOWED_VALUE: "yes", [SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV]: "still-hidden" } },
			});
			...
			if (previousBinding === undefined) delete process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV]; else process.env[SELESAI_SUBAGENT_EXTENSION_BINDINGS_ENV] = previousBinding;
		}
	});
