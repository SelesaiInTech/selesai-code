...
describe("shared temp paths", () => {
	it("uses the explicit temp root before shared paths resolve", () => {
		...
		try {
			...
			const result = spawnSync(process.execPath, ["--experimental-strip-types", "--input-type=module", "--eval", script], {
				encoding: "utf-8",
				env: { ...process.env, HOME: isolatedHome, USERPROFILE: isolatedHome, SELESAI_SUBAGENTS_TEMP_ROOT: override },
			});
	...
	it("isolates agent-dir profile writes from an inherited SELESAI_CODING_AGENT_DIR", () => {
...
`;
			const env = {
				...process.env,
				SELESAI_CODING_AGENT_DIR: callerAgentDir,
				SELESAI_SUBAGENTS_TEMP_ROOT: tempRoot,
			};
			delete env.SELESAI_SUBAGENTS_TEST_LOADER;
			...
			const isolatedAgentDir = path.join(tempRoot, "home", ".selesai", "agent");
