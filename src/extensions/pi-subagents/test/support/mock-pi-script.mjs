...
function writeStructuredOutputCapture(response) {
	...
	const outputPath = process.env.SELESAI_SUBAGENT_STRUCTURED_OUTPUT_CAPTURE;
	...
	if (Object.prototype.hasOwnProperty.call(response, "structuredOutputAcceptanceReport")) {
		const acceptancePath = process.env.SELESAI_SUBAGENT_STRUCTURED_OUTPUT_ACCEPTANCE_CAPTURE;
...
function writeToolDiagnostic(response) {
	...
	const required = JSON.parse(process.env.SELESAI_SUBAGENT_REQUIRED_TOOLS ?? "[]");
	...
	fs.writeFileSync(diagnosticPath, JSON.stringify({
		agent: process.env.SELESAI_SUBAGENT_CHILD_AGENT,
		required,
		available,
		missing,
	}), "utf-8");
...
function readRequiredToolsEnv() {
	try {
		const encoded = process.env.SELESAI_SUBAGENT_REQUIRED_TOOLS;
