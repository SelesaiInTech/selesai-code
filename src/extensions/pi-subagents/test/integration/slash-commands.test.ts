...
async function withTempProject<T>(prefix: string, fn: (root: string) => Promise<T>): Promise<T> {
	...
	fs.mkdirSync(path.join(root, ".selesai", "agents"), { recursive: true });
	fs.mkdirSync(path.join(root, ".selesai", "chains"), { recursive: true });
	try {
...
describe("slash command custom message delivery", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("/run accepts an agent without a task", async () => {
		...
		await commands.get("run")!.handler("explorer", ctx);
		await new Promise<void>((resolve) => setImmediate(resolve));

		assert.deepEqual(requestedParams, { agent: "explorer", task: "", clarify: false, agentScope: "both" });
	...
	it("/run finalizes the slash snapshot before the last UI redraw on success", async () => {
		...
		registerSlashCommands!(pi, createState(process.cwd()));
		await commands.get("run")!.handler("explorer inspect this", createCommandContext({
			hasUI: true,
			setStatus: (_key, text) => {
	...
	it("/parallel forwards inline output behavior config", async () => {
		...
		registerSlashCommands!(pi, createState(process.cwd()));
		await commands.get("parallel")!.handler("explorer[output=x.md,outputMode=file-only,reads=a.md+b.md,progress] -- Review", createCommandContext());

		assert.deepEqual(requestedParams, {
			tasks: [{ agent: "explorer", task: "Review", output: "x.md", outputMode: "file-only", reads: ["a.md", "b.md"], progress: true }],
			clarify: false,
			agentScope: "both",
		});
	});
	...
	it("/parallel no longer hard-blocks runs above the old 8-task limit before the executor responds", async () => {
		...
		registerSlashCommands!(pi, createState(process.cwd()));
		const args = Array.from({ length: 9 }, (_, index) => `explorer \"task ${index + 1}\"`).join(" -> ");
...
describe("saved chain slash command", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("/run and /chain accept dotted packaged runtime agent names", async () => {
		await withTempProject("pi-packaged-agent-slash-", async (root) => {
			fs.writeFileSync(path.join(root, ".selesai", "agents", "code-analysis.scout.md"), `---
name: scout
...
Inspect
`, "utf-8");
			fs.writeFileSync(path.join(root, ".selesai", "agents", "documentation.writer.md"), `---
name: writer
...
`);
			...
			const { params } = await captureSlashCommandParams("run-chain", "review-flow -- Shared task", root, () => {
				const userChainsDir = path.join(os.homedir(), ".selesai", "agent", "chains");
...
`, "utf-8");
	...
	it("/run-chain resolves saved outputSchema files at the command boundary", async () => {
		await withTempProject("pi-run-chain-schema-", async (root) => {
			const schemasDir = path.join(root, ".selesai", "chains", "schemas");
	...
	it("/chain parses a parenthesized parallel group into a { parallel: [...] } step", async () => {
		await withTempProject("pi-chain-group-slash-", async (root) => {
			for (const name of ["scout", "reviewer", "writer"]) {
				fs.writeFileSync(path.join(root, ".selesai", "agents", `${name}.md`), `---\nname: ${name}\ndescription: ${name}\n---\n\nBody\n`, "utf-8");
			}
...
describe("subagents-models slash command", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	...
	it("passes an optional builtin filter", async () => {
		const { params } = await captureSlashCommandParams("subagents-models", "explorer", process.cwd());
		assert.deepEqual(params, { action: "models", agent: "explorer" });
	});
	...
	it("suggests builtin agent names", async () => {
		await withIsolatedHome(async () => {
			...
			registerSlashCommands!(pi, createState(process.cwd()));
			const completions = commands.get("subagents-models")!.getArgumentCompletions!("ex") as Array<{ value: string; label: string }>;
			assert.deepEqual(completions.map((completion) => completion.value), ["explorer"]);
		});
	});
});
...
describe("subagent profiles slash commands", { skip: !available ? "slash-commands.ts not importable" : undefined }, () => {
	it("lists saved profiles", async () => {
		await withIsolatedHome(async () => {
			const profilesDir = path.join(process.env.HOME!, ".selesai", "agent", "profiles", "pi-subagents");
	...
	it("loads a saved profile into user settings", async () => {
		await withIsolatedHome(async () => {
			...
			const settings = JSON.parse(fs.readFileSync(path.join(process.env.HOME!, ".selesai", "agent", "settings.json"), "utf-8"));
	...
	it("can switch the current session model to the loaded profile worker model", async () => {
		await withIsolatedHome(async () => {
			...
			fs.mkdirSync(profilesDir, { recursive: true });
			fs.writeFileSync(path.join(profilesDir, "openai-codex.quota.json"), JSON.stringify({
				subagents: { agentOverrides: { builder: { model: "gpt-5.4:high" } } },
			}, null, 2));
			const sent: unknown[] = [];
			const commands = new Map<string, RegisteredSlashCommand>();
	...
	it("refreshes a provider model catalog", async () => {
		await withIsolatedHome(async () => {
			...
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".selesai", "agent", "profiles", "pi-subagents", "providers", "openai-codex.models.json")), true);
		});
	});
	...
	it("generates provider profiles", async () => {
		await withIsolatedHome(async () => {
			...
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".selesai", "agent", "profiles", "pi-subagents", "openai-codex.quota.json")), true);
			assert.equal(fs.existsSync(path.join(process.env.HOME!, ".selesai", "agent", "profiles", "pi-subagents", "openai-codex.quality.json")), true);
		});
	});
	...
	it("treats agent and scope selector cancellation as a silent no-op", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-subagents-admin-cancel-", async (root) => {
				...
				await commands.get("subagents")!.handler("builder model", createCommandContext({
					cwd: root,
					hasUI: true,
	...
	it("reports duplicate names as ambiguous without UI", async () => {
		await withIsolatedHome(async () => {
			await withTempProject("pi-subagents-admin-ambiguous-", async (root) => {
				fs.writeFileSync(path.join(root, ".selesai", "agents", "builder.md"), "---\nname: builder\ndescription: Project builder\n---\n\nWork.\n", "utf-8");
				const sent: unknown[] = [];
				const commands = registerAdmin(sent);
				await commands.get("subagents")!.handler("builder", createCommandContext({ cwd: root }));
				assert.equal(sent.length, 1);
				const content = (sent[0] as { content?: string }).content ?? "";
				assert.match(content, /Subagent 'builder' is ambiguous/);
		...
		await withIsolatedHome(async () => {
			await withTempProject("pi-subagents-admin-global-thinking-", async (root) => {
				...
				assert.deepEqual(JSON.parse(fs.readFileSync(settingsPath, "utf-8")).subagents, {
					...
					agentOverrides: { builder: { model: "test/new-model" } },
				});
			});
		});
	});
