...
describe("async execution utilities", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("forwards the installed Pi package root through the async runner environment", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		// The child resolves piPackageRoot as resolvePiPackageRoot() ??
		// resolveInstalledPiPackageRoot() at module load. In this fork the host
		// repo root is itself named @selesai/code, so the argv1 walk finds it
		// before the shim; mirror the child's expression for the expectation.
		const packageRoot = resolvePiPackageRoot() ?? resolveInstalledPiPackageRoot();
		assert.ok(packageRoot, "expected the test Pi package to be installed");
		mockPi.onCall({ echoEnv: [SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV] });
		...
		assert.deepEqual(JSON.parse(payload.results[0]?.output ?? "{}"), {
			[SELESAI_CODING_AGENT_PACKAGE_ROOT_ENV]: packageRoot,
		});
	});
	...
	it("persists the actual launch digest in async status and result metadata", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		// through, but SELESAI_SUBAGENT_REQUIRED_TOOLS excludes them so the 0.50 child
	...
	it("kills a wedged tool at the per-tool timeout with a tool-specific error before the run-level timeout", { skip: !isAsyncAvailable() ? "jiti not available" : process.platform === "win32" ? "timeout signal delivery intermittent on Windows CI" : undefined }, async () => {
		...
		process.env.SELESAI_SUBAGENT_TOOL_TIMEOUT_MS = "1000";
		try {
			executeAsyncChain(id, {
			...
			delete process.env.SELESAI_SUBAGENT_TOOL_TIMEOUT_MS;
		}
	});
	...
	it("background keeps a terminal answer authoritative over an earlier tool timeout", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		process.env.SELESAI_SUBAGENT_TOOL_TIMEOUT_MS = "600";
		try {
			executeAsyncChain(id, {
	...
	it("async dynamic fanout recomputes later child intercom targets by final flat index", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		mockPi.onCall({ output: "review-a", structuredOutput: { ok: "a" } });
		mockPi.onCall({ output: "review-b", structuredOutput: { ok: "b" } });
		mockPi.onCall({ echoEnv: ["SELESAI_SUBAGENT_INTERCOM_SESSION_NAME"] });
	...
	it("background event logs drop noisy message updates and cap child diagnostics", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		const previousMaxBytes = process.env.SELESAI_SUBAGENT_ASYNC_EVENTS_MAX_BYTES;
		process.env.SELESAI_SUBAGENT_ASYNC_EVENTS_MAX_BYTES = "900";
		try {
			mockPi.onCall({
				steps: [
					{
			...
			if (previousMaxBytes === undefined) delete process.env.SELESAI_SUBAGENT_ASYNC_EVENTS_MAX_BYTES;
			else process.env.SELESAI_SUBAGENT_ASYNC_EVENTS_MAX_BYTES = previousMaxBytes;
		}
	});
