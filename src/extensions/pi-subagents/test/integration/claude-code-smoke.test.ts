...
const enabled = process.env.SELESAI_SUBAGENTS_CLAUDE_CODE_SMOKE === "1";

test("maintainer Claude Code no-tools smoke", { skip: enabled ? undefined : "set SELESAI_SUBAGENTS_CLAUDE_CODE_SMOKE=1" }, async () => {
	const reportPath = process.env.SELESAI_SUBAGENTS_CLAUDE_CODE_SMOKE_REPORT;
	assert.ok(reportPath, "SELESAI_SUBAGENTS_CLAUDE_CODE_SMOKE_REPORT is required");
