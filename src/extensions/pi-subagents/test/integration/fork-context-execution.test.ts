...
describe("fork context execution wiring", { skip: !available ? "subagent executor not importable" : undefined }, () => {
	...
	function writeAgent(projectRoot: string, name: string, model: string): void {
		const filePath = path.join(projectRoot, ".selesai", "agents", `${name}.md`);
	...
	function writeProjectOverride(projectRoot: string, agentName: string, model: string): void {
		const settingsPath = path.join(projectRoot, ".selesai", "settings.json");
	...
	it("uses request cwd for management actions", async () => {
		...
		fs.mkdirSync(path.join(worktreeDir, ".selesai"), { recursive: true });
		...
		assert.equal(fs.existsSync(path.join(worktreeDir, ".selesai", "agents", "local-helper.md")), true);
		assert.equal(fs.existsSync(path.join(tempDir, ".selesai", "agents", "local-helper.md")), false);
	});
	...
	it("uses request cwd for project builtin overrides during management", async () => {
		...
		writeProjectOverride(tempDir, "commentator", "openai/gpt-5-main");
		writeProjectOverride(worktreeDir, "commentator", "openai/gpt-5-worktree");
		const executor = makeExecutor();

		try {
			const result = await executor.execute(
				"id",
				{ action: "get", agent: "commentator", cwd: "worktree" },
				new AbortController().signal,
				undefined,
				makeCtx(makeSessionManagerRecorder().manager),
			);
