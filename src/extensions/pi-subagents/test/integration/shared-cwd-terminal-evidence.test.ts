...
test("shared-cwd failure artifact preserves correlated allowlisted lifecycle once", () => {
	...
	const previous = process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR;
	process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR = path.join(root, "artifacts");
	...
	try {
		...
		observer.dispose();
		if (previous === undefined) delete process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR;
		else process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR = previous;
		fs.rmSync(runDir, { recursive: true, force: true });
		fs.rmSync(resultFile, { force: true });
...
test("shared-cwd evidence is failure-only and write errors retain safe log evidence", () => {
	...
	process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR = destination;
	...
	try {
		...
		delete process.env.SELESAI_SUBAGENTS_TERMINAL_EVIDENCE_DIR;
