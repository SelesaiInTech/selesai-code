...
test("context_pct defaults to the full tokens/window rendering", () => {
  ...
  assert.equal(stripAnsi(rendered.content), "12k/200k (6.2%)");
});
...
test("context_pct renders unknown usage after compaction", () => {
  ...
  assert.equal(stripAnsi(renderSegment("context_pct", full).content), "?/200k");
...
test("context_pct marks context estimates as approximate", () => {
  ...
  assert.equal(stripAnsi(renderSegment("context_pct", full).content), "~18k/272k (6.6%)");
