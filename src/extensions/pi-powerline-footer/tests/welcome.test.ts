...
test("discoverLoadedCounts ignores dangling skill symlinks", () => {
  ...
  const skillsDir = join(home, ".selesai", "agent", "skills");
...
test("welcome renders the initial system prompt token estimate", () => {
  ...
  const rendered = new WelcomeHeader("Model", "Provider", [], counts, "compact", [], 1900)
    .render(96)
    .join("\n")
    .replace(/\x1b\[[0-9;]*m/g, "");
  ...
  assert.match(indexSource, /new WelcomeHeader\(modelName, providerName, recentSessions, loadedCounts, "compact", newFeatures, initialContextTokens\)/);
...
test("getRecentSessions prefers cwd basename from session header", () => {
  withTemporaryHome((home) => {
    const sessionsDir = join(home, ".selesai", "agent", "sessions", "--Users-nico-dev-encoded-name--");
...
test("getRecentSessions falls back to encoded directory when header cwd is unusable", () => {
  withTemporaryHome((home) => {
    const root = join(home, ".selesai", "agent", "sessions");
    const cases = [
...
test("welcome discovery respects SELESAI_CODING_AGENT_DIR for agent-global files", () => {
  withTemporaryHome((home) => {
    ...
    try {
      ...
      mkdirSync(join(home, ".selesai", "agent"), { recursive: true });
      ...
      writeFileSync(join(home, ".selesai", "agent", "AGENTS.md"), "# Should not count\n");
      process.chdir(project);
...
test("getRecentSessions reads custom agent sessions and existing legacy sessions", () => {
  withTemporaryHome((home) => {
    ...
    try {
      ...
      const legacySessionDir = join(home, ".selesai", "sessions", "--legacy--");
