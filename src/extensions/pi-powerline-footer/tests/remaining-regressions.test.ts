...
test("startup welcome predicate respects powerline.welcome false", () => {
  ...
  // Guide display modes take precedence; the welcome overlay only shows when the
  // guide is off and shouldShowStartupWelcome passes.
  assert.match(source, /setupCustomEditor\(ctx\);/);
  assert.match(source, /if \(guideDisplayMode === "full"\)/);
  assert.match(source, /else if \(shouldShowStartupWelcome\(event\.reason, config\.welcome\)\)/);
});
