...
import { ENV_AGENT_DIR, getAgentDir, getFirstRunMarkerPath, markFirstRunComplete } from "../config.ts";
 ...
 * - onboarding was not completed before (first-run marker absent)
 *
 * Forks/rebrands (e.g. @selesai/code) run the same onboarding as the official
 * distribution. The gate uses a dedicated marker file instead of settings.json
 * existence so that bootstrapping a bundled default settings.json does not
 * suppress the dialog.
 */
export function shouldRunFirstTimeSetup(markerPath: string = getFirstRunMarkerPath()): boolean {
...
export async function showFirstTimeSetup(settingsManager: SettingsManager): Promise<void> {
	...
	return new Promise((resolve) => {
		...
		const finish = async (result: FirstTimeSetupResult | undefined) => {
			...
			// Mark onboarding complete whether the user confirmed or cancelled,
			// so the dialog does not reappear on every launch. A no-op result still
			// means the user saw and dismissed first-run UX.
