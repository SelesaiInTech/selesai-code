...
import { builtInExtensions } from "./core/built-in-extensions.ts";
...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "selesai -ne".';
...
// Bundled Selesai resources ship in dist and load through the same hooks as CLI paths.
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		const diagnostics: AgentSessionRuntimeDiagnostic[] = [
			...
			...resourceLoader.getExtensionDiagnostics().map((d) => ({
				type: (d.type === "error" ? "error" : "warning") as "error" | "warning",
				message: d.message,
			})),
		];
